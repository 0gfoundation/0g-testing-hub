...
5. **Triage** moves issues through `status:accepted` and `status:routed`, with `status:needs-info` when the repro needs tester input; a close carries exactly one `resolution:*` (`fixed` keeps reward credit, `rejected`/`duplicate` do not). The maintainer confirms/corrects the `area:*`/`product:*`, applies optional `rc:*`, and `systemic` when appropriate — routing, dedup, and reward eligibility are maintainer calls, never tester input.
